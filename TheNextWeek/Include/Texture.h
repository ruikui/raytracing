...
public:
    ...
    solid_texture(double r,double g,double b):solid_texture(color(r,g,b)) {}
...
public:
    ...
    checker_texture(color c1, color c2) :even(std::make_shared<solid_texture>(c1)), odd(std::make_shared<solid_texture>(c2)) {}

    virtual color value(double u, double v, const point3& p) const override
    {
        auto sines = std::sin(10 * p.x()) * sin(10 * p.y()) * sin(10 * p.z());
        if (sines < 0)
        {
...
public:
    ...
    image_texture(std::string filename):image_texture(filename.c_str()) {}
